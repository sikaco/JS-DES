...
DES.permute = function (inBlock, table, outblock_length) {
    var outBlock = "";
...
DES.IP = function () {
    this.TB_64b = this.permute(this.TB_64b, this.IP_Table, 64);
    this.TB_L = this.TB_64b.substr(0, 32);
    this.TB_R = this.TB_64b.substr(32, 32);
};

DES.Xor = function (bitStr1, bitStr2) {
    var outStr = "";
...
DES.subkeyCreate = function (j) {
    if (this.subkey == "") {
        if (this.key_b.length == 56) {
            this.key_b = this.key_b.replace(/(\d{7})/g, "$10");
        }
        this.key_56b = this.permute(this.key_b, this.PC1_Table, 56);
        this.C = this.key_56b.substr(0, 28);
        this.D = this.key_56b.substr(28, 28);
    }
    this.C = this.C.substr(this.LOOP_Table[j]) + this.C.substr(0, this.LOOP_Table[j]);
    this.D = this.D.substr(this.LOOP_Table[j]) + this.D.substr(0, this.LOOP_Table[j]);
    this.key_56b = this.C + this.D;
    this.subkey = this.permute(this.key_56b, this.PC2_Table, 48);
    return this.subkey;
};

DES.F_func = function (TBR_32b, subkey_48b) {
    var TB_48b = this.permute(TBR_32b, this.E_Table, 48);
    ...
    for (var i = 0; i < 8; i++) {
        ...
        temp_4b = this.S_Box[i][lineNum][columnNum].toString(2);
        result_32b += "0000".substr(temp_4b.length) + temp_4b;
    }
    return this.permute(result_32b, this.P_Table, 32);
};

DES.RIP = function () {
    this.TB_64b = this.TB_L + this.TB_R;
    this.TB_64b = this.permute(this.TB_64b, this.IPR_Table, 64);
};

DES.execute = function (inputText, key, mode) {
    this.executedText = "";
    this.TB_64b = "";
    this.TB_L = "";
    this.TB_R = "";
